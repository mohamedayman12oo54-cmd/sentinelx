...
`Identity ID` and `Organization ID` are immutable references. Everything else about a person — their name, their role, their permissions — is mutable business state that belongs in the database, fetched fresh on demand.
...
| Include `organization_name` for display convenience | Same staleness problem — organization names can change; only `organization_id` is a stable identifier |

---

## Consequences

- ✅ A token remains valid and correctly interpretable regardless of any profile, role, or organization-name change that happens after it was issued.
- ✅ Every layer downstream of Authentication (Authorization, Business Logic) is forced to treat Role and Organization as live, database-sourced facts — never as trusted-but-possibly-stale token claims.
- ✅ The token stays small, reducing header size on every request.
- ⚠️ Any endpoint that needs to *display* Name, Email, or Organization Name (e.g., `GET /me`) must perform an explicit database lookup using the `Identity ID` from the token — it cannot read this data directly off the JWT.
